...
class consultaModel extends Model {
	...
	public function getPlanes($familia) {
		if($familia==1){
			$sql = "SELECT id_clbkofi_tipo_plan, descripcion FROM aai_clbkofi_tipo_plan WHERE aai_flg_activo='t'
					AND descripcion LIKE '%CLARO MAX%' AND (descripcion NOT LIKE '%SF%' AND descripcion
					NOT LIKE '%SF CHIP%') ORDER BY 1 ASC";
		}
		else{
			$sql = "SELECT id_clbkofi_tipo_plan, descripcion FROM aai_clbkofi_tipo_plan WHERE aai_flg_activo='t' AND
					(descripcion LIKE '%SF%'OR descripcion LIKE '%SF CHIP%') ORDER BY 1 ASC";
		}
	...
	public function getInsert($nombre,$apellidopat,$apellidomat,$documento,$contacto,$observaciones,
		...
		if($arr[0]!='00000'){
			...
			return array($arr[0],$this->_db->lastInsertId());
		}
